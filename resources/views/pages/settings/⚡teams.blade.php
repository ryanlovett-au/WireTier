                ...
                <flux:table>
                    ...
                    @foreach ($teams as $team)
                        <flux:table.row :key="$team->id">
                            <flux:table.cell>
                                @php $tc = $teamColours[$team->colour] ?? '#6366f1'; @endphp
                                <div style="width:2.5rem;height:2.5rem;border-radius:0.5rem;background:{{ $tc }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                    <flux:icon name="{{ $team->icon }}" class="size-5 text-white" />
                                </div>
                            </flux:table.cell>
